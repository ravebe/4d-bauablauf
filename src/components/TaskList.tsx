...
interface Props { tasks: Task[]; setTasks: (t: Task[]) => void; api: any; viewerState: ViewerState; }
...
export default function TaskList({ tasks, setTasks, api, viewerState }: Props) {
  ...
  function gefiltert(): PsetAttr[] {
    ...
    return allAttrs.filter(a => a.name.toLowerCase().includes(attrSuche.toLowerCase()) || a.pset.toLowerCase().includes(attrSuche.toLowerCase())).slice(0, 10);
  }

  function selectAttr(a: PsetAttr) {
    setSelectedAttr(a); setAttrSuche(`${a.pset} › ${a.name}`);
    setShowAttrDrop(false); setAttrValue("");
    const w = attrMap[a.key] ? [...attrMap[a.key]].slice(0, 12) : [];
    setValueVorschlaege(w); setShowValueDrop(w.length > 0);
  }

  function typAendern(id: string, typ: TaskTyp) { setTasks(tasks.map(t => t.id === id ? { ...t, typ } : t)); }

  async function markiere(guids: string[]) {
    if (!api || !aktivesModellId || !guids.length) return;
    try {
      const nums = guids.map(Number).filter(n => !isNaN(n) && n >= 0);
      if (nums.length === guids.length) { await api.viewer.setSelection(nums); return; }
      const ids = await api.viewer.convertToObjectRuntimeIds(aktivesModellId, guids);
      await api.viewer.setSelection(ids);
    } catch {}
  }

  function taskKlick(taskId: string) {
    ...
    if (neu) {
      ...
      if (t?.objektGuids.length) markiere(t.objektGuids);
    }
  }

  async function perAttr(taskId: string) {
    if (!selectedAttr || !attrValue.trim()) { setMeldung({ text: "Attribut und Wert wählen.", typ: "err" }); return; }
    if (!api || !aktivesModellId) { setMeldung({ text: "Kein Modell aktiv.", typ: "err" }); return; }
    setLaden(true); setMeldung({ text: "⟳ Suche...", typ: "info" });
    try {
      const rohe = await api.viewer.getObjects(aktivesModellId);
      const allIds = parseObjectIds(rohe);
      if (!allIds.length) { setMeldung({ text: "Keine Objekte.", typ: "err" }); setLaden(false); return; }
      ...
      for (const obj of props) {
        ...
        for (const g of (obj?.properties || [])) {
          ...
          for (const p of (g?.properties || [])) {
            if (p?.name === selectedAttr.name && String(p?.value ?? "").trim().toLowerCase().includes(attrValue.trim().toLowerCase())) {
              treffer.push(rId); break;
            }
          }
        }
      }
      if (!treffer.length) { setMeldung({ text: "Keine Bauteile gefunden.", typ: "err" }); setLaden(false); return; }
      await api.viewer.setSelection(treffer);
      const guids = treffer.map(String);
      setTasks(tasks.map(t => t.id === taskId ? { ...t, objektGuids: [...new Set([...t.objektGuids, ...guids])] } : t));
      setMeldung({ text: `✓ ${treffer.length} Bauteile gefunden und markiert.`, typ: "ok" });
    } catch (e) { console.error(e); setMeldung({ text: "Fehler bei Suche.", typ: "err" }); }
  ...
  if (!aktivesModellId) return (
    <div style={{ padding: 16 }}>
      <div className="alert info">⚠ Kein Modell aktiv. Erstelle zuerst eine Simulation im Projektbereich.</div>
    </div>
  );

  return (
    <div className="tasklist-wrap" ref={panelRef} onClick={() => { setShowAttrDrop(false); setShowValueDrop(false); }}>

      {/* GANTT SECTION – kompakte Task-Liste */}
      <div className="gantt-section">
        <div className="gantt-section-header">
          Gantt · {tasks.length} Tasks
          {selektion.length > 0 && <span style={{ marginLeft: 8, color: "var(--tc-blue)", fontWeight: 400, textTransform: "none" }}>● {selektion.length} ausgewählt</span>}
        </div>
        {tasks.map(task => (
          <div
            key={task.id}
      ...
      {/* DETAIL SECTION – unterhalb der Liste */}
      <div className="detail-section" onClick={e => e.stopPropagation()}>
        {!aktTask ? (
          <div className="detail-empty">
            ↑ Task anklicken um Bauteile zuzuweisen
          </div>
        ) : (
          <>
            <div className="detail-header">
              ...
              <span style={{ fontSize: 10, color: "var(--tc-text-3)" }}>{aktTask.start} – {aktTask.end}</span>
            </div>

            {meldung && (
              <div style={{ padding: "0 12px 0" }}>
                <div className={`alert ${meldung.typ}`} style={{ marginTop: 8 }}>{meldung.text}</div>
              </div>
            )}
            ...
            <div className="detail-block">
              ...
              <div className="typ-btns">
                {(["neubau", "bestand", "abbruch"] as TaskTyp[]).map(typ => (
                  <button key={typ} className={`typ-btn ${aktTask.typ === typ ? `aktiv-${typ}` : ""}`}
            ...
            <div className="detail-block">
              <div className="detail-block-title">
                IFC-Attribut Filter
                {attrLaden && <span style={{ fontWeight: 400, textTransform: "none", marginLeft: 6, color: "var(--tc-text-3)" }}>⟳ Lade...</span>}
                {allAttrs.length > 0 && <span style={{ fontWeight: 400, textTransform: "none", marginLeft: 6, color: "var(--tc-text-3)" }}>{allAttrs.length} Attribute</span>}
              </div>

              <div className="ac-wrap" onClick={e => e.stopPropagation()}>
                <input className="ac-input" placeholder="PSet oder Attributname..."
                  value={attrSuche}
              ...
              <div className="ac-wrap" onClick={e => e.stopPropagation()}>
                <input className="ac-input" placeholder="Wert..."
                  value={attrValue} disabled={!selectedAttr}
                  onChange={e => {
                    setAttrValue(e.target.value);
                    if (selectedAttr && attrMap[selectedAttr.key]) {
                      const w = [...attrMap[selectedAttr.key]].filter(v => v.toLowerCase().includes(e.target.value.toLowerCase())).slice(0, 10);
                      setValueVorschlaege(w); setShowValueDrop(w.length > 0);
                    }
                  }}
                ...
                {showValueDrop && valueVorschlaege.length > 0 && (
                  <div className="ac-dropdown">
                    {valueVorschlaege.map(v => (
                      <div key={v} className="ac-item" onMouseDown={() => { setAttrValue(v); setShowValueDrop(false); }}>{v}</div>
                    ))}
                  </div>
                )}
              </div>
            ...
            <div className="detail-block">
              ...
              <div className={`sel-status ${selektion.length > 0 ? "aktiv" : ""}`}>
                {selektion.length > 0 ? `✓ ${selektion.length} Bauteil(e) im Viewer ausgewählt` : "Bauteile im 3D Viewer anklicken"}
              </div>
              <button className="tc-btn-secondary" style={{ width: "100%" }}
                onClick={() => perKlick(aktTask.id)} disabled={selektion.length === 0}>
            ...
            <div className="detail-block">
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                <div className="detail-block-title" style={{ marginBottom: 0 }}>{aktTask.objektGuids.length} Bauteile</div>
                {aktTask.objektGuids.length > 0 && (
                  <div style={{ display: "flex", gap: 5 }}>
                    <button style={{ fontSize: 10, background: "none", border: "1px solid var(--tc-border)", borderRadius: 3, padding: "2px 8px", cursor: "pointer", color: "var(--tc-blue)" }}
                      onClick={() => markiere(aktTask.objektGuids)}>👁 Markieren</button>
                    <button style={{ fontSize: 10, background: "none", border: "1px solid var(--tc-border)", borderRadius: 3, padding: "2px 8px", cursor: "pointer", color: "var(--tc-red)" }}
                      onClick={() => setTasks(tasks.map(t => t.id === aktTask.id ? { ...t, objektGuids: [] } : t))}>🗑 Alle</button>
                  </div>
                )}
              </div>
              {aktTask.objektGuids.length === 0
                ? <div style={{ fontSize: 11, color: "var(--tc-text-3)", textAlign: "center", padding: "6px 0" }}>Noch keine Bauteile verknüpft</div>
                : <div className="guid-list">
                  {aktTask.objektGuids.map(guid => (
                    <div key={guid} className="guid-row">
                      <span style={{ flex: 1 }}>{guid.slice(0, 22)}...</span>
                      <button className="guid-row-x" onClick={() => setTasks(tasks.map(t => t.id === aktTask.id ? { ...t, objektGuids: t.objektGuids.filter(g => g !== guid) } : t))}>✕</button>
                    </div>
                  ))}
                </div>
              }
